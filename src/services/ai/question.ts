...
export function GenerateAiQuestion({
  ...
  onFinish: (question: string) => Promise<void> | void;
}) {
  const previousMessage = previousQuestions.flatMap((q) => [
    {
      role: "user",
  ...
  return streamText({
    ...
    onFinish: async ({ text }) => await onFinish(text),
...
export function GenearateAiQuestionFeedback({
  question,
  answer,
}: {
  question: string;
