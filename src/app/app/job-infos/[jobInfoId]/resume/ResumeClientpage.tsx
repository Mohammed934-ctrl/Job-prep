...
type AnalysisResult = z.infer<typeof aiAnalyzeSchema>;
type Keys = Exclude<keyof AnalysisResult, "overallScore">;
...
export function ResumeClientPage({ jobInfoId }: { jobInfoId: string }) {
  ...
  async function handleFileUpload(file: File | null) {
    ...
    try {
      ...
      const res = await fetch("/api/ai/resumeanalyze", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {isLoading || aiAnalysis?.overallScore == null ? (
              ...
              <AccordionItem value={title} key={key}>
                ...
                <AccordionContent>
                  <div className="space-y-4">
                    {isLoading || category?.summary == null ? (
                      <>
                        <Skeleton />
                  ...
                  <div className="space-y-4 mt-4">
                    {isLoading || category?.feedback == null ? (
                      <>
...
function CategoryAccordionHeader({
  ...
  if (isLoading || score == null) {
    ...
    badge = <Badge variant="outline">Ok</Badge>;
  } else {
  ...
  return (
    <div className="flex items-start justify-between w-full">
      ...
      {isLoading || score == null ? <Skeleton /> : `${score}/10`}
    </div>
  );
}
...
function FeedbackItem({
  ...
  return (
    <div
      className={cn(
        "flex items-baseline gap-3 pr-5 py-5 rounded-lg",
        getcolors(),
      )}
    >
      <div>{getIcon()}</div>
